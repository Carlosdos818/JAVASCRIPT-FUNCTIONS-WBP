 ...
 */
function capitalizeAndReplace(product) {
...
function formatProducts(carousel, grid, sidebar) {
  const carouselProductsReformatted = carousel.map(capitalizeAndReplace);
  const gridProductsReformatted = grid.map(capitalizeAndReplace);
  const sidebarProductsReformatted = sidebar.map(capitalizeAndReplace);
